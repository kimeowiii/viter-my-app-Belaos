...
const DashboardSideNav = ({ isSideNavOpen, setIsSideNavOpen }) => {
  ...
  return (
    <>
      <nav
        className={`z-[1] w-[224px] transition-all ease-in-out duration-300 fixed top-16 left-0 ${
      ...
      >
        <span
          ...
          } -right-3 top-1 cursor-pointer bg-white size-[23px] rounded-full inline-flex items-center justify-center shadow-md group hover:bg-dashboard transition-all ease-in-out duration-200`}
        >
          <IndentDecrease
            onClick={toggleSideNav}
            className={`text-dashboard size-[18px] group-hover:text-white transition-all ease-in-out duration-300 cursor-pointer ${
